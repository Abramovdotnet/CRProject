...
class NPCCharacterCell: UICollectionViewCell {
    ...
    private func setupViews() {
        ...
        contentView.addSubview(desiredVictimIconContainer)
    }
        ...
        if !npc.isUnknown, let player = GameStateService.shared.getPlayer(),
           player.desiredVictim.isDesiredVictim(npc: npc) {
            ...
            Self.getCachedGlowImage(size: CGSize(width: iconSize + 12, height: iconSize + 12), color: UIColor.systemRed) { [weak self] glowImage in
                DispatchQueue.main.async {
                    self?.desiredVictimGlow.image = glowImage
                }
            }
        } else {
            UIView.animate(withDuration: animationDuration) {
    ...
    override func layoutSubviews() {
        ...
        // Обновляем glow для desiredVictimIndicator
        let desiredVictimGlowTag = 9999
        if let glowView = desiredVictimIconContainer.viewWithTag(desiredVictimGlowTag) as? UIImageView {
            let glowSize = iconSize + 12
            glowView.frame = CGRect(x: -6, y: -6, width: glowSize, height: glowSize)
            Self.getCachedGlowImage(size: CGSize(width: glowSize, height: glowSize), color: UIColor.systemRed) { glowImage in
                DispatchQueue.main.async {
